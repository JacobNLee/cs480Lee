...
bool Graphics::SendUniforms(Shader *m_shader, MenuVars* menu)
{
  ...
  glUniform4f(m_shader->m_diffuse1, menu->diffuse[0].x, menu->diffuse[0].y, menu->diffuse[0].z, 0.0);
  glUniform4f(m_shader->m_lightPosition1, menu->light_pos[0], menu->light_pos[1], menu->light_pos[2], 0.0);
  glUniform4f(m_shader->m_diffuse2, menu->diffuse[1].x, menu->diffuse[1].y, menu->diffuse[1].z, 0.0);
  //glUniform4f(m_shader->m_lightPosition2, menu->light_pos[1][0], menu->light_pos[1][1], menu->light_pos[1][2], 0.0);
...
//do in loop
  for (int index = 0; index < objectVector.size(); index++)
  {
    ...
    glUniform4f(m_shader->m_specular1, objectVector[index]->GetSpecular().x, objectVector[index]->GetSpecular().y, objectVector[index]->GetSpecular().z, objectVector[index]->GetSpecular().w);
